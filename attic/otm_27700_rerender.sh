...
for i in $*; do
	...
	echo "Rendering Zoom level ${i} for 27700-otm-c: "
	./render_list_geo.sh -n ${n} -m "27700-otm-c" -x -11 -X 2 -y 48.75 -Y 62.5 -z ${i} -Z ${i} -f
	[ $? != 0 ] && echo "Render failed on zoom level ${i} !" && exit 1
	echo "Done!"

	echo "Rendering Zoom level ${i} for 27700-otm-u: "
	./render_list_geo.sh -n ${n} -m "27700-otm-u" -x -11 -X 2 -y 48.75 -Y 62.5 -z ${i} -Z ${i} -f
	[ $? != 0 ] && echo "Render failed on zoom level ${i} !" && exit 1
	echo "Done!"
