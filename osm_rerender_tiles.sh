...
for i in $*; do
	...
	./render_list_geo.sh -n ${n} -m contoured -x -11 -X 8 -y 48.75 -Y 62.5 -z ${i} -Z ${i} -f
...
for i in $*; do
	...
	./render_list_geo.sh -n ${n} -m uncontoured -x -11 -X 8 -y 48.75 -Y 62.5 -z ${i} -Z ${i} -f
	[ $? != 0 ] && echo "Render failed on zoom level ${i} !" && exit 1
	echo "Done!"
...
for i in $*; do
	...
	./render_list_geo.sh -n ${n} -m opentopomapc -x -11 -X 8 -y 48.75 -Y 62.5 -z ${i} -Z ${i}	-f
	[ $? != 0 ] && echo "Render failed on zoom level ${i} !" && exit 1
	echo "Done!"
...
for i in $*; do
	...
	echo "Rendering Zoom level ${i} for opentopomapu: "
	./render_list_geo.sh -n ${n} -m opentopomapu -x -11 -X 8 -y 48.75 -Y 62.5 -z ${i} -Z ${i} -f
	[ $? != 0 ] && echo "Render failed on zoom level ${i} !" && exit 1
	echo "Done!"
